...
//t  当前时间
//b  初始值
//c  现在位置或者距离也就是distance
//d  总时间
//var cur=fx(t,b,c,d)
function starMove(obj,json, opations){
    clearInterval(obj.timer);
    opations=opations || {};
    opations.time=opations.time || 800;
    opations.type=opations.type || Tween.Bounce.easeOut;
    ...
    obj.timer=setInterval(function(){
        ...
        for(var name in json ){
            ...
            var cur=opations.type(opations.time*obj.n/iCount,star[name],dis[name],opations.time);
            if(name=='opacity'){
                obj.style.opacity=cur;
                obj.style.filter='opacity('+cur*100+')';
            }else{
